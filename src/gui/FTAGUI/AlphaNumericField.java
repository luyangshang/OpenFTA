...
public class AlphaNumericField extends JTextField {

    private static int _length; //holds number of characters allowed in text field
    private static boolean _restrictedLength; // flag to indicate whether the field is restricted in length;
...
/** Creates a Text Field with a specified number of columns, 
  ...
  */
    public AlphaNumericField(int cols, int length) {
	super(cols);
	_length = length;
	if(length < 0) {
	    _restrictedLength = false;
	} else {
	    _restrictedLength = true;
	}
    }
...
/** Creates a Text Field which only accepts positive Floats, 
  ...
  */
    public AlphaNumericField(int length) {
	super();
	_length =length;
	if(length < 0) {
	    _restrictedLength = false;
...
/** This method returns the Text from the Text Field.
  ...
  */
    ...
    static class AlphaNumericDocument extends PlainDocument {
